...
{	
	...
	if (delete == head && head == tail) // freelist has one Node before mybrk
	{
		...
		splitLastNode(delete,size);
		tail = head;
	}
	else if (delete == head)
	{
		if (head->size < (size + 2*sizeof(struct freelst)))
			head = head->next;
		else{
			splitLastNode(delete,size);
		}	
	...
	while (temp != tail)
	{
		if (temp->next == delete)
		{	
			if (delete->size < (size + 2*sizeof(struct freelst))){
...
void splitLastNode(struct freelst *delete, size_t size)
{
	...
	head->size = delete->size - (size + sizeof(struct freelst));
	head->next = temp->next;
	delete->size = size + sizeof(struct freelst);
	return;
}
...
{	
	brev->next = (char*)delete + size + sizeof(struct freelst);
	brev->next->size = delete->size - (size + sizeof(struct freelst));
